...
TestSetup = namedtuple(
    "TestSetup", "name architecture version host_python_tag build_python_tag"
)
    ...
    def __init__(self, resources):
        ...
        for tag, info in resources.items():
            if "source" not in info:
                raise KeyError("Resource {} missing 'source' field".format(tag))
...
class Resource(ExecEnvironment):
    ...
    def make_copy(self, destdir=None, symlinks=True):
        ...
        copy_tree(str(self.path), str(destdir), preserve_symlinks=symlinks)
        return new_env
        ...
        for arch in ARCHITECTURES:
            for build_installed in installed:
                ...
                for host_installed in installed:
                    ...
                    name = ":".join(
                        [arch.name, version, build_installed, host_installed]
                    )
                    setups.append(
                        TestSetup(
                            name, arch, version, host_python_tag, build_python_tag
                        )
                    )
    return setups
...
def build_python(request, crossenv_setup, tmp_path_factory):
    ...
    try:
        ...
        pytest.skip(
            "No build-python version {} available".format(crossenv_setup.version)
        )
    except FileNotFoundError:
        pytest.skip(
            "Build-python version {} registerd, but not found on "
...
def host_python(crossenv_setup):
    ...
    try:
        ...
        pytest.skip(
            "No Python version {} available for {}".format(
                crossenv_setup.version, crossenv_setup.architecture.name
            )
        )
    except FileNotFoundError:
        pytest.skip(
            "Python version {} for {} registerd, but not found on "
