...
def test_pip_install_numpy(tmp_path, host_python, build_python, python_version):
    ...
    crossenv.check_call(
        ["cross-pip", "--no-cache-dir", "install", "numpy==1.18.1", "pytest==5.3.5"]
    )
    ...
    host_python.setenv("PYTHONPATH", str(crossenv.cross_site_packages) + ":$PYTHONPATH")
    host_python.check_call(
        [
            host_python.binary,
            "-c",
...
def test_pip_install_bcrypt(tmp_path, host_python, build_python):
    ...
    host_python.setenv("PYTHONPATH", str(crossenv.cross_site_packages) + ":$PYTHONPATH")
    output = host_python.check_output(
        [
            host_python.binary,
            "-c",
            dedent(
