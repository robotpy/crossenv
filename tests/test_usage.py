...
def test_no_pip(tmp_path, host_python, build_python):
    crossenv = make_crossenv(tmp_path, host_python, build_python, "--without-pip")
    ...
    for python in python_variants:
        ...
        assert result.returncode != 0, "'%s -m pip' shouldn't succeed" % python
        assert "no module named pip" in result.stdout.lower()


def test_no_cross_pip(tmp_path, host_python, build_python):
    crossenv = make_crossenv(tmp_path, host_python, build_python, "--without-cross-pip")
